...
class GetReconstructionStatusInput(BaseModel):
    job_id: str
    timeout_sec: int = Field(default=30, ge=1, le=120)


class GetReconstructionStatusTool(BaseTool):
    name = "get_reconstruction_status"
    description = "Get the current status of an asynchronous 3D reconstruction job"
    input_model = GetReconstructionStatusInput
